...
class TemporalAttentionPooling(torch.nn.Module):
    def __init__(self, input_dim, mode="location", key_dim = 32, h_dim=32):
        super().__init__()
        self.mode = mode
        self.input_dim = input_dim
        self.h_dim = h_dim
        ...
        elif self.mode == "ienco":
            self.k_layer = torch.nn.Sequential(CustomTD(self.input_dim, self.key_dim), torch.nn.Tanh())#IENCO - KEY
            self.qins_layer = CustomTD(self.key_dim, 1, bias=False)#IENCO - query vector inside
        ...
        if self.mode in ["location", "ienco"]:
